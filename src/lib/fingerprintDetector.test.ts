...
describe('detectFromFingerprint', () => {
  ...
  describe('Linktree', () => {
    it('detecta "vim pelo Linktree" (case insensitive)', () => {
      ...
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Linktree',
        campanha: null,
      })
    })
    ...
    it('detecta mensagem completa com "vim pelo Linktree"', () => {
      const messages: Message[] = [
        { content: 'Ola, vim pelo Linktree e quero saber mais sobre as camisas personalizadas', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Linktree',
        campanha: null,
  ...
  describe('Google/Site', () => {
    ...
    it('detecta mensagem completa com "vim pelo Site"', () => {
      const messages: Message[] = [
        { content: 'Ola, vim pelo Site e quero saber mais sobre as camisas personalizadas', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Google/Site',
        campanha: null,
    ...
    it('detecta "vim pelo google"', () => {
      const messages: Message[] = [
        { content: 'vim pelo google', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Google/Site',
        campanha: null,
    ...
    it('detecta "pelo site" em mensagem curta', () => {
      const messages: Message[] = [
        { content: 'pelo site', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Google/Site',
        campanha: null,
    ...
    it('NAO detecta "pelo site" em mensagem longa', () => {
      const messages: Message[] = [
        { content: 'eu nao comprei pelo site nao, foi presencial na loja mesmo', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toBeNull()
    })
  })
  ...
  describe('Meta CTWA', () => {
    ...
    it('NAO detecta "tenho interesse..." quando is_from_me=true', () => {
      const messages: Message[] = [
        { content: 'tenho interesse e queria mais informacoes', is_from_me: true },
      ]
      expect(detectFromFingerprint(messages)).toBeNull()
    })
  })
  ...
  describe('Meta Redirect', () => {
    ...
    it('NAO detecta "recebi o seu contato" quando is_from_me=false', () => {
      const messages: Message[] = [
        { content: 'recebi o seu contato aqui', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toBeNull()
    })
  })
  ...
  describe('Instagram', () => {
    ...
    it('detecta "Vim pelo Instagram"', () => {
      const messages: Message[] = [
        { content: 'Vim pelo Instagram', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
    ...
    it('detecta "Vi no Instagram"', () => {
      const messages: Message[] = [
        { content: 'Vi no Instagram', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Instagram Organico',
        campanha: null,
    ...
    it('detecta "pelo instagram" em mensagem curta', () => {
      const messages: Message[] = [
        { content: 'pelo instagram', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Instagram Organico',
        campanha: null,
    ...
    it('NAO detecta "pelo instagram" em mensagem longa (>50 chars)', () => {
      const messages: Message[] = [
        { content: 'eu nao comprei pelo instagram nao, foi presencial na loja', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toBeNull()
    })
  })
  ...
  describe('Indicacao', () => {
    ...
    it('detecta "Indicacao" com acento', () => {
      const messages: Message[] = [
        { content: 'Indicacao', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
    ...
    it('detecta "Recebi a indicacao de vcs"', () => {
      const messages: Message[] = [
        { content: 'Recebi a indicacao de vcs', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Indicacao',
        campanha: null,
  ...
  describe('Meta Ads (via anuncio)', () => {
    it('detecta "vi no anuncio"', () => {
      const messages: Message[] = [
        { content: 'vi no anuncio', is_from_me: false },
      ]
      expect(detectFromFingerprint(messages)).toEqual({
        origem: 'Meta Ads (via anuncio)',
        campanha: null,
      })
    })
  ...
  describe('prioridade e edge cases', () => {
    it('primeiro match vence: array com multiplos patterns retorna o primeiro', () => {
      const messages: Message[] = [
        { content: 'vim pelo Linktree', is_from_me: false },
        { content: 'vim pelo Site', is_from_me: false },
      ]
    ...
    it('ignora mensagens is_from_me=true para patterns de cliente', () => {
      const messages: Message[] = [
        { content: 'vim pelo Linktree', is_from_me: true },
      ]
      expect(detectFromFingerprint(messages)).toBeNull()
    })
