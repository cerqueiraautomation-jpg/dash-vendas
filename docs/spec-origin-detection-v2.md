...
## Instagram Direct (adicionado 2026-04-07)

O CRM recebeu atualização que permite atender DMs do Instagram direto no CRM. A nova origem foi adicionada aos Tiers 1 e 3 e tem bucket próprio "Novos Instagram" na segmentação do frontend (agrupando também Instagram Orgânico e Instagram Stories, que foram removidas de `ORIGENS_TRAFEGO`).

**Observação sobre o match Bling → contato IG:** contatos com `origin='instagram_direct'` têm `phone` no formato `ig:<instagram_id>` (não é telefone real), então o match por telefone no `sync-bling-vendas` não funciona pra eles. O match por nome também não é confiável porque o `full_name` no IG é o nome exibido do perfil (pode ser username/apelido), enquanto no Bling é o nome fiscal do cliente. Para vendas IG Direct serem ligadas corretamente a conversas IG Direct, o CRM precisa mesclar/unificar o contato IG com o contato WhatsApp/phone do cliente.

**Cor:** `#E1306C` (oficial Instagram).
